...
import { loginSuccess, updateProfile } from '../../../store/slices/authSlice';

const Profile = ({  onProfileComplete, isProfileComplete }) => {
  ...
  const { token , user } = useSelector((state) => state.auth);
    const [profileExists, setProfileExists] = useState(user.profileComplete);
...
//         email: data.emailId || user.emailId || '',
//         phone: data.phone || '',
//         age: data.age || '',
//         weight: data.weight || '',
...
const handleSave = async () => {
  ...
  try {
    ...
    console.log("Saving profile for patientId:", user.patientId);
    ...
    if( user.patientId != null && user.firstName == "" && user.lastName == "" ){
      ...
      const Response = await fetch(
        ...
        if (Response.ok) {
          ...
          console.error("Failed to create profile:", Response.status, data);
     ...
     const body = JSON.stringify({
          ...
          console.log("BODY ==> " + body );
    const Response = await fetch(
        `${import.meta.env.VITE_PATIENT_SERVICE_BASE_URL}/${user.patientId}`,
        {
          method: "PUT",
          headers: {
            "Authorization": `${token}`,
        ...
        <div className="flex items-center gap-3">
          {isEditing ? (
            ...
            >
              ...
              {!isProfileComplete ? 'Save Profile' : 'Edit Profile'}
            </button>
          )}
        </div>
      </div>
